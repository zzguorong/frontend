...
export default {
  ...
  data() {
    return {
      ...
      // 鸟瞰图颜色类别
      aerialviewGroups: [
        { color: "#B47878", label: "建筑" },
        { color: "#06E6E6", label: "天空" },
        { color: "#04C803", label: "树" },
        { color: "#8C8C8C", label: "道路" },
        { color: "#04FA07", label: "草地" },
        { color: "#EBFF07", label: "人行道" },
        { color: "#96053D", label: "人" },
        { color: "#787846", label: "地面" },
        { color: "#8FFF8C", label: "山" },
        { color: "#CCFF04", label: "植物" },
        { color: "#0066C8", label: "汽车" },
        { color: "#3DE6FA", label: "水" },
        { color: "#FF09E0", label: "房子" },
        { color: "#0907E6", label: "海" },
        { color: "#7009FF", label: "田地" },
        { color: "#FF3D06", label: "铁轨" },
        { color: "#A09614", label: "沙子" },
        { color: "#8C8C8C", label: "大厦" },
        { color: "#FF1F00", label: "小径" },
        { color: "#99FF00", label: "跑道" },
        { color: "#FF5200", label: "桥" },
        { color: "#FF6600", label: "小山丘" },
        { color: "#FF00F5", label: "巴士" },
        { color: "#FF0014", label: "货车" },
        { color: "#FFB8B8", label: "塔楼" },
        { color: "#0047FF", label: "路灯" },
        { color: "#00FF52", label: "飞机" },
        { color: "#FFEB00", label: "船" },
        { color: "#00FF5C", label: "林荫" },
        { color: "#00B8FF", label: "游泳池" },
        { color: "#0ABED4", label: "湖泊" },
        { color: "#2900FF", label: "车流" },
      ],
      // 表单验证状态
      formErrors: {
        viewType: false,
  ...
  methods: {
    ...
    applyStoredParams() {
      ...
      if (p.basemapUrl) {
        ...
        // 更新缩略图第一个位置（底图）
        this.$set(this.thumbnails, 0, p.basemapUrl);
      }
      if (p.semanticImgUrl) {
        this.semanticImgUrl = p.semanticImgUrl;
        // 更新缩略图第二个位置（语义分割图）
        this.$set(this.thumbnails, 1, p.semanticImgUrl);
    ...
    // 查看更多
    goToDetail() {
      this.$router.push("/example/generate/detail?id=1");
    },
    // 自动识别
    automaticRecognition() {
      ...
      // 调用后端接口获取 base64 图
      preprocessSegment({
        image_id: this.semanticImgUrlId,
      }).then((res) => {
        ...
        if (base64Str) {
          ...
          // 4. 同步更新缩略图第 1 张（语义分割图）
          this.$set(this.thumbnails, 1, base64Str);
        ...
        getImageDetail(requestId)
          .then((res) => {
            ...
            if (status === "completed") {
              ...
              const imageUrls = res.data[0].generated_images.map((item) => {
                return item.url;
              });
              ...
              imageUrls.forEach((url, idx) => {
                ...
                if (targetIndex < this.thumbnails.length) {
                  // 使用 Vue.set 以保持响应式
                  this.$set(this.thumbnails, targetIndex, url);
                } else {
                  // 如果超出原有长度，则追加
    ...
    selectThumbnail(index) {
      ...
      if (index !== 0 && index !== 1) {
        ...
        // this.$router.push("/example/generate/detail?id=" + thumb);
      }
      ...
      if (this.thumbnails[index]) {
        this.previewImage = this.thumbnails[index];
        console.log("选择缩略图:", index, "预览图片已更新");
    ...
    deleteThumbnail(index) {
      this.$confirm(
        ...
        .then(() => {
          // 确认删除：将对应位置设为null
          this.$set(this.thumbnails, index, null);
    ...
    // 底图上传成功处理
    onBasemapUpload(uploadData, type) {
      ...
      if (type === 1) {
        ...
        this.$message.success("底图上传成功！");
        // 更新缩略图第一个位置（底图）
        this.$set(this.thumbnails, 0, imageUrl);
        // 更新主预览图
        this.previewImage = imageUrl;

        // 将底图自动传输到语义分割图位置（索引1）
        this.$set(this.thumbnails, 1, imageUrl);
      } else if (type === 2) {
      ...
      blobUrlToBase64(this.previewImage).then((base64) => {
    ...
    temporaryStore() {
      // 检查是否有预览图片
      if (!this.previewImage) {
        this.$message.warning("预览区域没有图片，无法暂存");
        return;
      }

      // 将预览图片传输到语义分割图位置（索引1）
      this.$set(this.thumbnails, 1, this.previewImage);

      // 更新语义分割图URL
      this.semanticImgUrl = this.previewImage;

      this.$message.success("图片已暂存到语义分割图框！");
      console.log("暂存成功，将预览图片传输到语义分割图框:", this.previewImage);
    },
