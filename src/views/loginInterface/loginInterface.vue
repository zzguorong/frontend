        ...
        <div class="form-wrapper">
          ...
          <div v-if="activeTab === 'phone'" class="tab-content phone-login">
            ...
            <div class="agree-checkbox-wrapper">
              ...
              我已阅读并接受<span class="agree-word">服务条款</span>和<span
                class="agree-word"
                >隐私政策</span
              >
            </div>
...
<script>
import { getWechatLoginUrl } from "@/api/generate";
export default {
  name: "LoginInterface",
  data() {
    return {
  ...
  methods: {
    handleLogin() {
      ...
      //       if (res.data.wechat_login_url) {
      //         window.location.href = res[0]; // 跳转到微信扫码页面
      //       } else {
      //         this.$message.error("获取微信登录链接失败");
      //       }
      //     });
      //   } catch (err) {
