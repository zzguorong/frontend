...
export default {
  ...
  methods: {
    ...
    async generateWechatLoginQRCode() {
      ...
      // 如果当前域名是https://gaiass.com，则回调域名为https://gaiass.com，否则为https://www.gaiass.com
      const redirectDomain = window.location.hostname === 'gaiass.com' ? 'https://gaiass.com' : 'https://www.gaiass.com';
      // 初始化微信登录二维码
      new WxLogin({
        ...
        redirect_uri: encodeURIComponent(
          `${redirectDomain}/auth/wechat/callback?wechat_binding=true` // 授权成功后跳转的路径，必须是经过URL编码的
        ), // 扫完码授权成功跳转到的路径
        fast_login: 0,
